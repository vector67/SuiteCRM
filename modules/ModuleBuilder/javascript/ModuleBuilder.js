        ...
        ModuleBuilder = {
            init: function () {
                ...
                var mp = ModuleBuilder.mainPanel = new YAHOO.widget.Layout('mblayout', {
                    border: false, height: viewHeight - heightOffset - 40, //autoHeight: true
                    //frame: true,
                    units: [//ModuleBuilder.tree, ModuleBuilder.tabPanel,
                        {
                            position: 'center', body: 'mbcenter', scroll: true
                        }, {
                            position: "left",
                            header: "Tree",
                            collapse: true,
                ...
                var correctW = function () {
                    ...
                    if (typeof Studio2 != "undefined") Studio2.resizeDivs();
                    if (typeof resizeDDLists == "function") resizeDDLists();
                };
            ...
            layoutValidation: {
                popup_window: null, popup: function () {
            ...
            }, //Custom logic hide/show manager
            customHideLogic: {
                popup_window: false,
                reverted: false,
                params: {},
                ...
                browse: function (module, layout, subpanel) {
                    ...
                    if (!ModuleBuilder.customHideLogic.popup_window) {
                        ModuleBuilder.customHideLogic.popup_window = new YAHOO.SUGAR.AsyncPanel('customHideLogicWindow', {
                            width: 900, close: true, constraintoviewport: true, fixedcenter: false
                        });
                    }
                    ...
                    ModuleBuilder.customHideLogic.popup_window.setHeader(module_str + ' : Rules');
                    ModuleBuilder.customHideLogic.popup_window.setBody("test");
                    ModuleBuilder.customHideLogic.popup_window.render(document.body);
                    ModuleBuilder.customHideLogic.params = {
                        module: 'ModuleBuilder',
                        histAction: 'browse',
                        action: 'customHideLogic',
                        view_package: ModuleBuilder.MBpackage,
                        view_module: module,
                        view: layout,
                        subpanel: subpanel
                    };
                    ModuleBuilder.customHideLogic.popup_window.load(ModuleBuilder.paramsToUrl(ModuleBuilder.customHideLogic.params), "GET",
                        response => {
                            Array.from(this.popup_window.body.querySelectorAll("script")).forEach(oldScript => {
                    ...
                    ModuleBuilder.customHideLogic.popup_window.show();
                    ModuleBuilder.customHideLogic.popup_window.center();
                },
                addRule: function (rule) {
                    var ruleTemplate = YAHOO.util.Dom.get('ruleTemplate');
                    var ruleContainer = YAHOO.util.Dom.get('ruleContainer');
                    var clone = ruleTemplate.cloneNode(true); // true tells it to do a deep clone
                    var ruleNumber = (rule && rule.ruleNumber ? rule.ruleNumber : ModuleBuilder.customHideLogic.ruleId);
                    var parentId = 'rule' + ruleNumber;
                    clone.setAttribute('id', parentId);
                    clone.classList.remove('hide');
                    var elements = Array.from(clone.getElementsByClassName('addParentElementReference'));
                    for (var i = 0; i < elements.length; i++) {
                        var element = elements[i];
                        element.classList.remove('addParentElementReference');
                        element.setAttribute('data-parent-id', parentId);
                        if (rule && rule.hasOwnProperty(element.getAttribute('name'))) {
                            if (element.classList.contains("multiple")) {
                                var options = rule[element.getAttribute('name')];
                                for (var j = 0, l = element.options.length, o; j < l; j++) {
                                    o = element.options[j];
                                ...
                                element.value = rule[element.getAttribute('name')];
                            }
                        }
                    }
                    if (rule && rule.ruleType === 'role') {
                        clone.querySelector('[name=ruleSecurityGroupId]').classList.add('hide');
                    } else if (rule && rule.ruleType === 'security_group') {
                        clone.querySelector('[name=ruleRoleId]').classList.add('hide');
                    }
                    var is = clone.querySelector('#ruleCloseButton');
                    is.setAttribute('id', 'ruleCloseButton' + ruleNumber);
                    ruleContainer.append(clone);
                    ModuleBuilder.customHideLogic.ruleId = parseInt(ruleNumber) + 1;
                },
                editRule: function (input) {
                    var ruleNumber = input.getAttribute('data-parent-id').substr(4);
                    const ruleElements = ["ruleFieldName", "ruleComparatorType", "ruleType", "ruleRoleId", "ruleSecurityGroupId"];
                    var ruleBody = {ruleNumber};
                    ruleElements.forEach(value => {
                        var val = $("[name=" + value + "][data-parent-id=rule" + ruleNumber + "]").val();
                        if (val) {
                            ruleBody[value] = val;
                        }
                    });
                    if (ModuleBuilder.customHideLogic.loading === 0) {
                        document.getElementById("ruleSyncedImage").classList.add('hide');
                        document.getElementById("ruleSyncingImage").classList.remove('hide');
                    }
                    ModuleBuilder.customHideLogic.loading += 1;
                    ModuleBuilder.customHideLogic.params.histAction = 'save';
                    ModuleBuilder.customHideLogic.params.action = 'customHideLogic';
                    ModuleBuilder.customHideLogic.params.rule = JSON.stringify(ruleBody);
                    ModuleBuilder.asyncRequest(ModuleBuilder.customHideLogic.params, function (response) {
                        ModuleBuilder.customHideLogic.loading -= 1;
                        if (ModuleBuilder.customHideLogic.loading === 0) {
                            document.getElementById("ruleSyncedImage").classList.remove('hide');
                            document.getElementById("ruleSyncingImage").classList.add('hide');
                        }
                    });
                },
                removeRule: function (input) {
                    ...
                    ModuleBuilder.customHideLogic.loading += 1;
                    ModuleBuilder.customHideLogic.params.histAction = 'remove';
                    ModuleBuilder.customHideLogic.params.action = 'customHideLogic';
                    ModuleBuilder.customHideLogic.params.rule = ruleNumber;
                    var rule = document.getElementById(input.getAttribute('data-parent-id'));
                    rule.remove();
                    ModuleBuilder.asyncRequest(ModuleBuilder.customHideLogic.params, function () {
                        ModuleBuilder.customHideLogic.loading -= 1;
                        if (ModuleBuilder.customHideLogic.loading === 0) {
                            document.getElementById("ruleSyncedImage").classList.remove('hide');
                            document.getElementById("ruleSyncingImage").classList.add('hide');
            ...
            history: {
                popup_window: false, reverted: false, params: {}, browse: function (module, layout, subpanel) {
                    subpanel = subpanel ? subpanel : "";
                    if (!module && ModuleBuilder.module != "undefined") {
                        module = ModuleBuilder.module;
                    }
                    if (!ModuleBuilder.history.popup_window) {
                        ModuleBuilder.history.popup_window = new YAHOO.SUGAR.AsyncPanel('histWindow', {
                            width: 300, draggable: true, close: true, constraintoviewport: true, fixedcenter: false
                        });
                    }
                    var module_str = module;
                    if (typeof SUGAR.language.languages['app_list_strings']['moduleList'][module] != 'undefined') {
                ...
                isDirty: false, saving: false, hideFailedMesage: false, intended_view: {
                    url: null, successCall: null
                }, current_view: {
                    url: null, successCall: null
                }, save_url_for_current_view: null, popup_window: null, setupState: function () {
                    ...
                    if (!saveBtn) {
                        ...
                        if (mbForm) var mbButtons = mbForm.getElementsByTagName("input");
                        if (mbButtons) {
            ...
            navigate: function (url) {
                ...
                if (typeof (successCall) != 'function') {
                    if (ModuleBuilder.callInProgress) return;
                ...
                if (ModuleBuilder.checkForErrors(o)) return false;
                ...
                for (var maj in ajaxResponse) {
                    ...
                    if (!comp) {
                        ...
                        for (i in tabs) {
                            if (tabs[i].get && tabs[i].get("id") == maj) comp = tabs[i];
                        }
                    }
                    ...
                    if (name == 'mbwest') { //refresh package_tree!
                        ...
                        if (!comp) {
                            ...
                            comp = new YAHOO.SUGAR.ClosableTab({
                                content: "<div class='bodywrapper'><script>ModuleBuilder.scriptTest=true;</script>" + ((maj == 'center') ? "<div>" + ajaxResponse[maj].crumb + "</div>" : "") + ajaxResponse[maj].content + "</div>",
                            ...
                            if (!ModuleBuilder.scriptTest) SUGAR.util.evalScript(ajaxResponse[maj].content);
                        } else {
                            ...
                            if (ajaxResponse[maj].title != "no_change") comp.set('label', ajaxResponse[maj].title);
                ...
                if (SUGAR.util.isLoginPage(o.responseText)) return true;
                ...
                if (typeof (successCall) == 'undefined') {
                    ...
                    if (formname != "dropdown_form" && formname != "popup_form" && // user came from studio/fields layout by ajax urls
                        ((urlVars.module == 'ModuleBuilder' && urlVars.action == 'modulefields' && urlVars.view_package == 'studio') || // user refresh the page or came from direct url
                ...
                Connect.asyncRequest(Connect.method, Connect.url, {
            ...
            preloader: {
                ...
                on: function (title, message) {
                    ...
                    // show message
                    YAHOO.SUGAR.MessageBox.show({
                        title: title, msg: message, width: 500,
            ...
            helpRegister: function (name) {
                ...
                if (name == 'default') name = ModuleBuilder.helpDefault;
            ...
            addModule: function (MBpackage) {
                ...
                ModuleBuilder.asyncRequest('module=Administration&action=RebuildRelationship&silent=true', function () {
                });
                ModuleBuilder.asyncRequest('module=Administration&action=RebuildDashlets&silent=true', function () {
                });
                ...
                if (typeof (formsWithFieldLogic) != 'undefined') formsWithFieldLogic = 'undefined';
                ...
                if (typeof (type) == 'undefined') type = 0; else if (type == "studio") {
                ...
                for (var i = 0; i < tabs.length; i++) {
                    if (tabs[i].get("id") == id) return tabs[i];
                }
                return null;
                ...
                Connect.asyncRequest(Connect.method, Connect.url + '&' + url, {
                    success: callback,
                ...
                ModuleBuilder.asyncRequest('module=ModuleBuilder&action=get_app_list_string&key=' + value + '&view_package=' + ModuleBuilder.MBpackage + '&view_module=' + ModuleBuilder.module, ModuleBuilder.dropdownChangedCallback);
